...
CDS_PRODUCTS = {
    ...
    # Oropouche virus proteins (L, M, S segments)
    "RNA_polymerase": "L",
    "RNA-dependent_RNA_polymerase": "L",
    "polyprotein": "M",
    "membrane_polyprotein": "M",
    "nucleoprotein": "N",
    "non-structural_protein": "NSs",
    "nonstructrural_protein": "NSs",
    "nonstructural_protein": "NSs",
}
...
def parse_args():
    ...
    parser.add_argument('--annotation-choice', type=int, default=2, choices=[0, 1, 2],
                        help='Annotation choice: 1=CDS, 2=mature_protein (default), 0=case-by-case (interactive)')
    parser.add_argument('--name-field', type=str, default='product',
                        help='Field to use as CDS name (default: product)')
    ...
    with open(output_gff_file, "w") as out:
        ...
        for record in SeqIO.parse(genbank_file, "genbank"):
            ...
            for feature in record.features:
                ...
                # Use 'gene' instead of 'CDS' for Nextclade compatibility
                feature_type = "gene" if feature.type == "CDS" else feature.type
                out.write(
                    f"{seqid}\tGenBank\t{feature_type}\t{start}\t{end}\t.\t{strand}\t.\t{attribute}\n"
                )
...
if __name__=="__main__":
    ...
    # Use CLI argument for annotation choice (default: 2 = mature_protein)
    annotation_choice = args.annotation_choice
    if any([len(feat)>1 for feat in all_cds.values()]):
        print(f"\nUsing annotation choice: {annotation_choice} (1=CDS, 2=mature_protein, 0=interactive)")
    ...
    # Use CLI argument for name field (default: 'product')
    name_choice = args.name_field
    print(f"Using CDS name field: '{name_choice}'")
    if name_choice and name_choice not in available_attributes:
        print(f"WARNING: '{name_choice}' not in available attributes {available_attributes}, falling back to 'product'")
        name_choice = 'product' if 'product' in available_attributes else ''
